...
function (_PubSub) {
  ...
  _proto._onPromiseEnd = function _onPromiseEnd() {
    ...
    Promise.all(uploadPromiseList).then(function () {
...
function getToken(userData) {
  if (userData.appId) {
    ...
    var _url2 = new URL('//api.polyv.net/inner/v3/upload/video/create-upload-token');

    Object.entries(_data2).forEach(function (_ref) {
      var key = _ref[0],
          value = _ref[1];

      _url2.searchParams.set(key, value);
    });
    return fetch(_url2, {
      method: 'GET'
    });
  ...
  var url = new URL("//api.polyv.net/v2/uploadvideo/" + userData.userid + "/token");
